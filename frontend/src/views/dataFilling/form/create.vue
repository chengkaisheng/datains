...
export default {
  ...
  computed: {
    ruleUpdateFormList() {
      const filterList = filter(this.formSettings && this.formSettings.forms, f => !!(f.settings && f.settings.updateRuleCheck))
      return map(filterList, f => { return f.settings && f.settings.name })
    },
      ...
      if (dsMap) {
        forEach(keys(dsMap), type => {
          if (type === 'mysql' || type === 'mariadb') {
            _types.push({
              name: dsMap[type][0] && dsMap[type][0].typeDesc,
              type: type,
              options: dsMap[type]
            })
          }
  ...
  methods: {
    ...
    openEditCommitRule() {
      this.tempForms = map(this.formSettings.forms, f => {
        return {
          id: f.id,
          updateRuleCheck: !!(f.settings && f.settings.updateRuleCheck),
          typeName: f.typeName,
          name: f.settings && f.settings.name
        }
      })
