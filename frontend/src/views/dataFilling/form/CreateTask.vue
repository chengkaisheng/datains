...
export default {
  ...
  methods: {
    ...
    validateMin(rule, value, callback) {
      if (this.id && value && value.getTime() === (this.sourceForm.startTime && this.sourceForm.startTime.getTime())) { return callback() }
    ...
    validateMax(rule, value, callback) {
      if (this.id && value && value.getTime() === (this.sourceForm.endTime && this.sourceForm.endTime.getTime())) { return callback() }
      if (!value) return callback()
      const val = new Date(value)
    ...
    validatePublishMin(rule, value, callback) {
      if (this.id && value && value.getTime() === (this.sourceForm && this.sourceForm.publishStartTime && this.sourceForm.publishStartTime.getTime())) { return callback() }
      if (!value) return callback()
      const val = new Date(value)
      if (val.getTime() < Date.now() - 60 * 1000) {
        return callback(new Error('不能小于当前时间'))
    ...
    validatePublishMax(rule, value, callback) {
      if (this.id && value && value.getTime() === (this.sourceForm.publishEndTime && this.sourceForm.publishEndTime.getTime())) { return callback() }
      if (!value) return callback()
      const val = new Date(value)
      if (val.getTime() < Date.now()) {
        return callback(new Error('不能小于当前时间'))
