...
export default {
  ...
  computed: {
    ...
    forms: function() {
      if (this.param && this.param.forms) {
    ...
    columns: function() {
      ...
      forEach(filter(this.forms, f => !f.removed), f => {
        if (f.type === 'dateRange') {
          _list.push({
            props: f.settings && f.settings.mapping && f.settings.mapping.columnName1,
            label: f.settings && f.settings.name,
            date: true,
            dateType: f.settings && f.settings.dateType ? f.settings.dateType : (f.settings && f.settings.enableTime ? 'datetimerange' : 'daterange'),
            type: f.type,
            multiple: !!(f.settings && f.settings.multiple),
            rangeIndex: 0
          })
          _list.push({
            props: f.settings && f.settings.mapping && f.settings.mapping.columnName2,
            label: f.settings && f.settings.name,
            date: true,
            dateType: f.settings && f.settings.dateType ? f.settings.dateType : (f.settings && f.settings.enableTime ? 'datetimerange' : 'daterange'),
            type: f.type,
          ...
          _list.push({
            props: f.settings && f.settings.mapping && f.settings.mapping.columnName,
            label: f.settings && f.settings.name,
            date: f.type === 'date',
            dateType: f.type === 'date' ? (f.settings && f.settings.dateType ? f.settings.dateType : (f.settings && f.settings.enableTime ? 'datetime' : 'date')) : undefined,
            type: f.type,
            multiple: !!(f.settings && f.settings.multiple)
          })
        }
