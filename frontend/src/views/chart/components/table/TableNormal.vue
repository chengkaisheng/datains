      ...
      if (this.chart.customAttr) {
        ...
        if (customAttr.color) {
          ...
          this.table_header_class.fontFamily = this.canvasStyleData.fontFamily
            ? this.canvasStyleData.fontFamily
            : ''
      ...
      if (this.chart.customStyle) {
        ...
        if (customStyle.text) {
          ...
          this.title_class.fontWeight = customStyle.text.isBolder
            ? 'bold'
            : 'normal'
          this.title_class.fontFamily = customStyle.text.fontFamily
        ...
        if (columnIndex === 0 && x.length > 0) {
          ...
          if (columnIndex >= x.length) {
            const values = data.map((item) => {
              const valueStr = item[column.property]
            ...
            if (!values.every((value) => isNaN(value))) {
              ...
              var accuracy = 0
              var isPercentage = ''
              var foundObject = yaxis.find(function(obj) {
                return obj.datainsName === column.property
              })
