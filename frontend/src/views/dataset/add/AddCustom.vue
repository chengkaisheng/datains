  ...
  methods: {
    ...
    calHeight() {
      ...
      setTimeout(function () {
        ...
        that.height =
          currentHeight - 56 - 15 - 26 - 25 - 43 - 16 - 37 - 20 - 10;
      }, 10);
    },
    ...
    getChecked(tableCheckedField) {
      ...
      if (
        tableCheckedField.checkedFields &&
        tableCheckedField.checkedFields.length > 0
      ) {
        if (
          !this.checkedList.some(
            (ele) => ele.tableId === tableCheckedField.tableId
          )
        ) {
    ...
    getData() {
      ...
      if (this.checkedList.length > 0) {
        const table = {
          ...
          dataSourceId: this.param.tableId
            ? this.param.table.dataSourceId
            : this.table.dataSourceId,
          type: "custom",
    ...
    getUnionData() {
      if (this.checkedList && this.checkedList.length > 0) {
        // 根据第一个选择的数据集找到关联视图
        post(
          "dataset/union/listByTableId/" + this.checkedList[0].tableId,
          {}
        ).then((response) => {
          // console.log(response)
