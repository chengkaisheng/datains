<template>
  <view class="custom-tree-select-content">
    ...
    >
      <view class="left">
        <view v-if="selectedListBaseinfo.length" class="select-items">
          <view
            class="select-item"
      ...
      <view class="right">
        <uni-icons
          v-if="!selectedListBaseinfo.length || !clearable"
          type="bottom"
          size="14"
          color="#999"
        ></uni-icons>
        <uni-icons
          v-if="selectList.length && treeData.length && clearable"
          type="clear"
          size="24"
          color="#c0c4cc"
...
export default {
  ...
  methods: {
    ...
    close() {
      ...
      this.$emit('finish', this.selectedListBaseinfo)
    },
    // 弹窗状态变化 包括点击回显框和遮罩
    change(data) {
      if (data.show) {
