...
import DeMainContainer from '@/components/datains/DeMainContainer.vue'
import DeAsideContainer from '@/components/datains/DeAsideContainer.vue'
import GridTable from '@/components/gridTable/index.vue'
import { getForm, searchFormMyTasks, searchTable } from './dataFilling/form/dataFilling'
import EditFormData from './dataFilling/form/EditFormData.vue'
      ...
      if (row.valueId) {
        getForm(row.formId).then(res => {
          ...
          forEach(this.forms, f => {
            if (f.type === 'dateRange') {
              dateFormatColumns.push(f.settings && f.settings.mapping && f.settings.mapping.columnName1)
              dateFormatColumns.push(f.settings && f.settings.mapping && f.settings.mapping.columnName2)
            } else {
              if (f.type === 'date') {
                dateFormatColumns.push(f.settings && f.settings.mapping && f.settings.mapping.columnName)
              }
            }
          })
