...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    toOidcLogin: function toOidcLogin() {
      var info = this.authInfo;
      if (!info || __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_keys___default()(info).length === 0) {
        this.$warning('缺少oidc信息');
        return;
      }
      if (!info.authParam || __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_keys___default()(info.authParam).length === 0) {
        this.$warning('缺少oidc认证参数');
        return;
      }
      this.fillParam(info);
      var urlStr = info.authEndpoint + this.buildParam(this.param);
      var url = urlStr;
      //   const url = encodeURIComponent(urlStr)
      window.location.href = url;
    },
    buildParam: function buildParam(param) {
      if (param === null || __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_keys___default()(param).length === 0) return '';
      var result = '';
      __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_keys___default()(param).forEach(function (key) {
        result += '&' + key + '=' + param[key];
      });
      result = result.replace('&', '?');
      return result;
    },
    fillParam: function fillParam(info) {
      __WEBPACK_IMPORTED_MODULE_0_babel_runtime_core_js_object_assign___default()(this.param, info.authParam);
      var callBack = this.origin + '/sso/callBack';
      this.param.redirect_uri = encodeURIComponent(callBack);
      this.param.scope = encodeURIComponent(this.param.scope);
    },
    requestAuthInfo: function requestAuthInfo() {
      var _this = this;

      var url = '/plugin/oidc/authInfo';
...
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return (_vm.show)?_c('div'):_vm._e()}
var staticRenderFns = []
