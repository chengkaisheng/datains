package io.datains.provider.query.dm;
...
import io.datains.plugins.common.constants.Dm8Constants;
...
@Service("dm8Query")
public class Dm8QueryProvider extends QueryProvider{
	...
	public String createQuerySQL(String table, List<DatasetTableField> fields, boolean isGroup, Datasource ds,
		...
		SQLObj tableObj = SQLObj.builder()
                .tableName((table.startsWith("(") && table.endsWith(")")) ? table : String.format(Dm8Constants.KEYWORD_TABLE, table))
        ...
        if (CollectionUtils.isNotEmpty(fields)) {
            for (int i = 0; i < fields.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(f.getExtField()) && f.getExtField() == 2) {
                    ...
                    originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), f.getOriginName());
                } else {
                    originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), f.getOriginName());
                }
                ...
                if (f.getDeExtractType() == 1) {
                    if (f.getDeType() == 2 || f.getDeType() == 3) {
                        fieldName = String.format(Dm8Constants.UNIX_TIMESTAMP, originField) + "*1000";
                    } else {
                        fieldName = originField;
                    }
                } else if (f.getDeExtractType() == 0) {
                    if (f.getDeType() == 2) {
                        fieldName = String.format(Dm8Constants.CAST, originField, Dm8Constants.DEFAULT_INT_FORMAT);
                    } else if (f.getDeType() == 3) {
                        fieldName = String.format(Dm8Constants.CAST, originField, Dm8Constants.DEFAULT_FLOAT_FORMAT);
                    } else if (f.getDeType() == 1) {
                        fieldName = String.format(Dm8Constants.STR_TO_DATE, originField, Dm8Constants.DEFAULT_DATE_FORMAT);
                    } else {
                        fieldName = originField;
                    }
                } else {
                    if (f.getDeType() == 1) {
                        String cast = String.format(Dm8Constants.CAST, originField, Dm8Constants.DEFAULT_INT_FORMAT) + "/1000";
                        fieldName = String.format(Dm8Constants.FROM_UNIXTIME, cast, Dm8Constants.DEFAULT_DATE_FORMAT);
	...
	public String getSQL(String table, List<ChartViewFieldDTO> xAxis, List<ChartViewFieldDTO> yAxis,
		 ...
		 SQLObj tableObj = SQLObj.builder()
	        ...
	        if (CollectionUtils.isNotEmpty(xAxis)) {
	            for (int i = 0; i < xAxis.size(); i++) {
	                ...
	                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
	                    ...
	                    originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
	                } else {
	                    originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
	                }
	                String fieldAlias = String.format(SQLConstants.FIELD_ALIAS_X_PREFIX, i);
	        ...
	        if (CollectionUtils.isNotEmpty(yAxis)) {
	            for (int i = 0; i < yAxis.size(); i++) {
	                ...
	                if (ObjectUtils.isNotEmpty(y.getExtField()) && y.getExtField() == 2) {
	                    ...
	                    originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
	                } else {
	                    originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), y.getOriginName());
	                }
	        ...
	        SQLObj tableSQL = SQLObj.builder()
	                .tableName(String.format(Dm8Constants.BRACKETS, sql))
            ...
            for (int i = 0; i < xAxis.size(); i++) {
                ...
                if (ObjectUtils.isNotEmpty(x.getExtField()) && x.getExtField() == 2) {
                    ...
                    if (x.getDeType() == 2 || x.getDeType() == 3) {
                        originField = String.format(Dm8Constants.CAST, String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName()), MysqlConstants.DEFAULT_FLOAT_FORMAT);
                    } else {
                        originField = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), x.getOriginName());
                    }
                }
	...
	private String getYWheres(ChartViewFieldDTO y, String originField, String fieldAlias) {
        ...
        if (CollectionUtils.isNotEmpty(y.getFilter()) && y.getFilter().size() > 0) {
            y.getFilter().forEach(f -> {
                String whereTerm = transDm8FilterTerm(f.getTerm());
                String whereValue = "";
                ...
                if (StringUtils.equalsIgnoreCase(f.getTerm(), "null")) {
                    ...
                    whereValue = String.format(Dm8Constants.WHERE_VALUE_VALUE, f.getValue());
                }
	...
	private SQLObj getYFields(ChartViewFieldDTO y, String originField, String fieldAlias) {
        ...
        if (StringUtils.equalsIgnoreCase(y.getOriginName(), "*")) {
            fieldName = Dm8Constants.AGG_COUNT;
        } else if (SQLConstants.DIMENSION_TYPE.contains(y.getDeType())) {
            fieldName = String.format(Dm8Constants.AGG_FIELD, y.getSummary(), originField);
        } else {
            if (StringUtils.equalsIgnoreCase(y.getSummary(), "avg") || StringUtils.containsIgnoreCase(y.getSummary(), "pop")) {
                String cast = String.format(Dm8Constants.CAST, originField, y.getDeType() == 2 ? Dm8Constants.DEFAULT_INT_FORMAT : Dm8Constants.DEFAULT_FLOAT_FORMAT);
                String agg = String.format(Dm8Constants.AGG_FIELD, y.getSummary(), cast);
                fieldName = String.format(Dm8Constants.CAST, agg, Dm8Constants.DEFAULT_FLOAT_FORMAT);
            } else {
                String cast = String.format(Dm8Constants.CAST, originField, y.getDeType() == 2 ? Dm8Constants.DEFAULT_INT_FORMAT : Dm8Constants.DEFAULT_FLOAT_FORMAT);
                fieldName = String.format(Dm8Constants.AGG_FIELD, y.getSummary(), cast);
            }
        }
	...
	private SQLObj getXFields(ChartViewFieldDTO x, String originField, String fieldAlias) {
        ...
        if (x.getDeExtractType() == 1) {
            if (x.getDeType() == 2 || x.getDeType() == 3) {
                ...
                fieldName = String.format(Dm8Constants.DATE_FORMAT, originField, format);
            } else {
                fieldName = originField;
            }
        } else {
            if (x.getDeType() == 1) {
                ...
                if (x.getDeExtractType() == 0) {
                    ...
                    String cast = String.format(Dm8Constants.CAST, originField, Dm8Constants.DEFAULT_INT_FORMAT) + "/1000";
                    String from_unixtime = String.format(Dm8Constants.FROM_UNIXTIME, cast, Dm8Constants.DEFAULT_DATE_FORMAT);
                    fieldName = String.format(Dm8Constants.DATE_FORMAT, from_unixtime, format);
                }
            } else {
                fieldName = originField;
            }
	...
	public String transExtFilterList(SQLObj tableObj, List<ChartExtFilterRequest> requestList) {
        ...
        for (ChartExtFilterRequest request : requestList) {
            ...
            String whereName = "";
            String whereTerm = transDm8FilterTerm(request.getOperator());
            String whereValue = "";
            ...
            if (ObjectUtils.isNotEmpty(field.getExtField()) && field.getExtField() == 2) {
                ...
                originName = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), field.getOriginName());
            } else {
                originName = String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), field.getOriginName());
            }
            ...
            if (field.getDeType() == 1) {
                if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5) {
                    whereName = String.format(Dm8Constants.STR_TO_DATE, originName, Dm8Constants.DEFAULT_DATE_FORMAT);
                }
                if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                    String cast = String.format(Dm8Constants.CAST, originName, Dm8Constants.DEFAULT_INT_FORMAT) + "/1000";
                    whereName = String.format(Dm8Constants.FROM_UNIXTIME, cast, Dm8Constants.DEFAULT_DATE_FORMAT);
                }
                ...
                if (field.getDeExtractType() == 0 || field.getDeExtractType() == 5) {
                    whereName = String.format(Dm8Constants.CAST, originName, Dm8Constants.DEFAULT_FLOAT_FORMAT);
                }
                if (field.getDeExtractType() == 1) {
                    whereName = String.format(Dm8Constants.UNIX_TIMESTAMP, originName);
                }
                if (field.getDeExtractType() == 2 || field.getDeExtractType() == 3 || field.getDeExtractType() == 4) {
                    whereName = originName;
                }
            ...
            if (StringUtils.containsIgnoreCase(request.getOperator(), "in")) {
                ...
                if (request.getDatasetTableField().getDeType() == 1) {
                    ...
                    whereValue = String.format(Dm8Constants.WHERE_BETWEEN, startTime, endTime);
                } else {
                    whereValue = String.format(Dm8Constants.WHERE_BETWEEN, value.get(0), value.get(1));
                }
            } else {
                whereValue = String.format(Dm8Constants.WHERE_VALUE_VALUE, value.get(0));
            }
            list.add(SQLObj.builder()
	 ...
	 * @return
	 */
	public String transDm8FilterTerm(String term) {
        switch (term) {
            case "eq":
                return " = ";
            case "not_eq":
        ...
        for (ChartFieldCustomFilterDTO request : requestList) {
            ...
            if (StringUtils.equalsIgnoreCase(request.getFilterType(), "enum")) {
                ...
                for (ChartCustomFilterItemDTO filterItemDTO : filter) {
                    ...
                    String whereTerm = transDm8FilterTerm(filterItemDTO.getTerm());
                    String whereValue = "";
                    ...
                    if (StringUtils.equalsIgnoreCase(filterItemDTO.getTerm(), "null")) {
                        ...
                        whereValue = String.format(Dm8Constants.WHERE_VALUE_VALUE, value);
                    }
                    list.add(SQLObj.builder()
                            .whereField(whereName)
                            .whereTermAndValue(whereTerm + whereValue)
	...
	private String calcFieldRegex(String originField, SQLObj tableObj) {
        ...
        for (DatasetTableField ele : calcFields) {
            originField = originField.replaceAll("\\[" + ele.getId() + "]",
                    String.format(Dm8Constants.KEYWORD_FIX, tableObj.getTableAlias(), ele.getOriginName()));
        }
        return originField;
    }
