...
public class FillFormInfoController {
    ...
    @PostMapping("/selectForm/{goPage}/{pageSize}")
    public Pager<List<FillFormInfoVo>> selectForm(@PathVariable int goPage, @PathVariable int pageSize,
                                              @RequestBody FillFormInfoReqVo request) {
        Page<Object> page = PageHelper.startPage(goPage, pageSize, true);
        return PageUtils.setPageInfo(page, fillFormInfoService.selectForm(request));
    }

    @ApiOperation("查询树")
    @PostMapping("/tree")
    public List<FillFormInfoVo> tree(@RequestBody FillFormInfoReqVo request) {
        return fillFormInfoService.tree(request);
    }
