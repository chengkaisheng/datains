...
import io.datains.fill.mapper.DataFillTaskMapper;
...
import io.datains.fill.request.DataFillUserTaskSearchRequest;
        ...
        if (StringUtils.isNotBlank(request.getName())) {
            ...
            if (insert) {
                if (dataFillTaskMapper.countByExample(example) > 0) {
                    DataInsException.throwException(Translator.get("I18N_DATA_FILL_TASK_EXIST"));
                }
