...
public class DataFillDataService {
    ...
    public Datasource getDataSource(String datasourceId, boolean withCreatePrivileges) {
        ...
        if (StringUtils.equals("default-built-in", datasourceId)) {
            ...
            if (!withCreatePrivileges) {
                ...
                if (ds.getEnableDataFill() && ds.getEnableDataFillCreateTable()) {
                    ds.setConfiguration(new String(Base64.getDecoder().decode(ds.getConfiguration())));
                } else {
                    return null;
                }
            }
        ...
        if (ds == null) {
            DataInsException.throwException(Translator.get("I18N_DATA_FILL_DATASOURCE_NOT_EXIST"));
        }
        return ds;
    }

    public static void setLowerCaseRequest(Datasource ds, DatasourceProvider datasourceProvider, ExtDDLProvider extDDLProvider, DatasourceRequest datasourceRequest) throws Exception {
    ...
    public DataFillFormTableDataResponse listData(DataFillFormTableDataRequest searchRequest, boolean withLogs) throws Exception {
        ...
        if (StringUtils.isNotBlank(searchRequest.getPrimaryKeyValue())) {
            whereSql = extDDLProvider.whereSql(dataFillForm.getTableName(), Collections.singletonList(pk));
            datasourceRequest.setTableFieldWithValues(Collections.singletonList(new DatasourceRequest.TableFieldWithValue()
    ...
    public void deleteRowData(String formId, String id) throws Exception {
        deleteRowData(formId, Collections.singletonList(id));
    }
                ...
                if (StringUtils.equalsIgnoreCase(field.getType(), "input")) { //input框支持unique
                    if (field.getSettings().isUnique() && data.get(name) != null) {
                        ...
                        if (uniqueMap.get(name).contains(data.get(name).toString())) {
                            ...
                            DataInsException.throwException("[" + extTableFields.get(name).getSettings().getName() + "] 值: " + data.get(name) + " 不能重复");
                        } else {
            ...
            for (ExtTableField field : fields) {
                ...
                if (!StringUtils.equalsIgnoreCase(field.getType(), "dateRange") && data.get(name) == null) {
                    if (field.getSettings().isRequired()) {
                        DataInsException.throwException("[" + field.getSettings().getName() + "] 不能为空");
                    }
                ...
                if (StringUtils.equalsIgnoreCase(field.getType(), "input")) { //input框支持unique
                    if (field.getSettings().isUnique() && data.get(name) != null) {
                        ...
                        if (count > 0) {
                            DataInsException.throwException("[" + extTableFields.get(uniqueField.getFiledName()).getSettings().getName() + "] 值: " + data.get(name) + " 在数据库中已存在, 不能重复");
                        }
    ...
    private String updateRowData(String rowId, Map<String, Object> data, DatasourceRequest.TableFieldWithValue pkField, List<ExtTableField> fields, Map<String, TableField> tableFieldMap, DataFillFormWithBLOBs dataFillForm, DatasourceRequest datasourceRequest, JdbcProvider jdbcProvider, ExtDDLProvider extDDLProvider) throws Exception {
        ...
        if (CollectionUtils.isEmpty(searchFields)) {
            //DataInsException.throwException("没有修改");
            skip = true;
        }
    ...
    public void importExcelData(MultipartFile file, String formId) throws Exception {
        ...
        if (excelHeaders.size() != fields.size()) {
            DataInsException.throwException("模板字段个数不匹配");
        }
        ...
        for (List<String> excelDatum : excelData) {
            ...
            for (int i = 0; i < fields.size(); i++) {
                    ...
                    switch (field.getSettings().getMapping().getType()) {
                        ...
                        default:
                            if (StringUtils.equalsIgnoreCase(field.getType(), "select") && !field.getSettings().isMultiple() || StringUtils.equalsIgnoreCase(field.getType(), "radio")) {
                                ...
                                if (!has) {
                                    DataInsException.throwException("[" + field.getSettings().getName() + "] 值: " + excelRowData + " 不在范围内");
                                }
                                ...
                                if (CollectionUtils.isNotEmpty(list)) {
                                    ...
                                    for (String str : list) {
                                        ...
                                        if (has) {
                                            ...
                                            DataInsException.throwException("[" + field.getSettings().getName() + "] 输入值[" + str + "]不在范围内");
                                        }
                                    }
                                    if (CollectionUtils.isEmpty(result)) {
                                        DataInsException.throwException("[" + field.getSettings().getName() + "] 输入值不在范围内");
                                    }
                                }
                ...
                } catch (DataInsException e) {
                    DataInsException.throwException(e.getMessage());
                } catch (Exception e) {
                    DataInsException.throwException("[" + field.getSettings().getName() + "] 值: " + excelRowData + " 格式解析错误: " + e.getMessage());
                }
            }
            ...
            dateRangeCheckMap.forEach((key, list) -> {
                if (list.get(0) == null && list.get(1) != null) {
                    DataInsException.throwException("[" + dateRangeNameMap.get(key) + "（开始/结束）] 不能只有一个为空");
                } else if (list.get(0) != null && list.get(1) == null) {
                    DataInsException.throwException("[" + dateRangeNameMap.get(key) + "（结束/结束）] 不能只有一个为空");
                } else if (list.get(0) != null && list.get(1) != null) {
                    if (list.get(0) > list.get(1)) {
                        DataInsException.throwException("[" + dateRangeNameMap.get(key) + "（结束）] 不能早于 [" + dateRangeNameMap.get(key) + "（开始）]");
                    }
                }
            });
